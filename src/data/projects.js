...
export const projects = [
...

  {
    ...
    highlights: [
      ...
      'term, vote and log reach disk before the node answers the RPC that changed them, through a Storage with three backends: files for a process, memory for the tests, nothing for the browser demo.',
      'compaction replaces the applied prefix of the log with a snapshot of the state machine, and a follower that needs entries which no longer exist is caught up with InstallSnapshot instead of one entry per round trip.',
      'reads go through a read index: the leader pins the read to its commit index and waits for a majority to answer a round of heartbeats, so a leader that has been partitioned away refuses the read rather than serving a stale copy.',
      'kill the leader process and a new leader is elected within a few seconds, with every committed key intact. kill every process and the cluster comes back from disk with the same store.',
    ],
